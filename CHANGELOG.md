...
- Grok live STT finalizes when the mic stops (existing Stream audio while recording toggle). Ordinary dictation skips LLM cleanup and screenshots; turn off Preserve exact wording to opt back into cleanup. The live socket serializes PCM, then PTT `Finalize`, then `audio.done`. Streaming sends `format=true` (ITN) with language. Grok uses `filler_words=false`. Custom vocabulary is sent as `keyterm` query params.
